...
const useCaSdkAuth = async (inject: any) => {
    const initializeCA = async (provider: EthereumProvider) => {
        try {
            if (!caSDK) {
                ...
                await caSDK.init()
                balance = (await caSDK?.getUnifiedBalances()!).map((b: any) => ({
                  local: undefined,
                  ...b
                }));
    ...
    console.log('useCaSdkAuth called', {inject})
    // @ts-ignore
    // const injectedProvider = window.ethereum;

    await initializeCA(inject).then(async () => {
      balance = (await caSDK?.getUnifiedBalances()!).map((b: any) => ({
        local: undefined,
        ...b
      }));
